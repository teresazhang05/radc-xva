...
void fill_delta_gamma(std::vector<float>& A, int64_t N, int64_t S, const radc::Config& cfg,
  ...
  for (int64_t n = 0; n < N; ++n) {
    ...
    float* out_row = A.data() + static_cast<size_t>(n) * static_cast<size_t>(S);
    for (int64_t s = 0; s < S; ++s) {
      ...
      out_row[s] = static_cast<float>(notional * (linear + cross + quad + lambda * phi * phi));
    }
  }
}
...
int run(const radc::Config& cfg) {
  ...
  for (int64_t epoch = 0; epoch < cfg.run.epochs; ++epoch) {
    ...
    fill_delta_gamma(local, cfg.buffer.N, cfg.buffer.S, cfg, epoch, rank, &local_notional);
    ...
    radc::run_protocol_epoch(MPI_COMM_WORLD, cfg, epoch, A_view, rt, out, pm, total_notional);
  ...
  if (rank == 0) {
    ...
    std::cout << "bench_xva_delta_gamma checksum=" << checksum << std::endl;
  }
