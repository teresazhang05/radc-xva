...
void fill_synth_lowrank(std::vector<float>& A, int64_t N, int64_t S, const radc::Config& cfg,
  ...
  for (int64_t n = 0; n < N; ++n) {
    ...
    for (int64_t s = 0; s < S; ++s) {
      ...
      row[s] = static_cast<float>(acc + noise * eps);
    }
  }
}
...
int run(const radc::Config& cfg) {
  ...
  for (int64_t epoch = 0; epoch < cfg.run.epochs; ++epoch) {
    ...
    fill_synth_lowrank(local, cfg.buffer.N, cfg.buffer.S, cfg, epoch, rank);
  ...
  if (rank == 0) {
    ...
    std::cout << "bench_allreduce_matrix checksum=" << checksum << std::endl;
  }
